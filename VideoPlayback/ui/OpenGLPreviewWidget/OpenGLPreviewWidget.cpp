...
{
	if (m_uiWidth != width || m_uiHeight != height)
	{
		m_uiWidth = width;
		m_uiHeight = height;
	...
	uint8_t* yuyv422 = new uint8_t[width * height * 2]{ 0 };
	memcpy(yuyv422, data.data(), width * height * 2);
	uint8_t* yuv420 = new uint8_t[width * height * 3 / 2]{ 0 };
	//ConvertYUYV422ToYUV420(yuyv422, width, height, yuv420, yuv420 + width * height, yuv420 + width * height * 5 / 4);
	ConvertYUV422pToYUV420p(yuyv422, width, height, yuv420);
	m_YUV420Data = QByteArray((char*)yuv420, width * height * 3 / 2);
